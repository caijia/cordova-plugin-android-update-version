...
public class WaterProgress extends View {
  ...
  public WaterProgress(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    TypedArray a =
        context.obtainStyledAttributes(attrs, R.styleable.WaterProgress, defStyleAttr, 0);
    init(a);
  }

  @TargetApi(Build.VERSION_CODES.LOLLIPOP)
  public WaterProgress(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    super(context, attrs, defStyleAttr, defStyleRes);
    TypedArray a =
        context.obtainStyledAttributes(attrs, R.styleable.WaterProgress, defStyleAttr, defStyleRes);
    init(a);
  }
  ...
  public synchronized void setWave(float wave) {
    this.wave = wave;
    postInvalidate();
  }
  ...
  private void setDiam(float diam) {
    if (diam < dp2px(MIN_DIAM_SIZE)) diam = dp2px(MIN_DIAM_SIZE);
    this.diam = diam;
  }
  ...
  private void buildWave(Path path) {
    // wave path
    path.reset();
    ...
    path.quadTo(
        -diam * 3f / 4 + offset,
        levelHeight + quarterDiam * wave,
        -diam / 2f + offset,
        levelHeight);
    path.quadTo(-quarterDiam + offset, levelHeight - quarterDiam * wave, offset, levelHeight);
    path.quadTo(
        quarterDiam + offset, levelHeight + quarterDiam * wave, diam / 2f + offset, levelHeight);
    path.quadTo(
        diam * 3f / 4 + offset,
        levelHeight - quarterDiam * wave,
        diam + strokeWidth * 2 + offset,
        levelHeight);
